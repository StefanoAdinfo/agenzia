...
<section class="container py-5">
    <h2 class="mb-4">Archivio categoria: <?php single_cat_title(); ?></h2>
                        ...
                        <footer class="it-card-related it-card-footer d-flex justify-content-between align-items-center">
                            <div class="it-card-taxonomy">
                                ...
                                if (!empty($categories) && !is_wp_error($categories)) :
                                    $cat = $categories[0];
                                ?>
                                    <a href="<?php echo esc_url(get_category_link($cat->term_id)); ?>" class="it-card-category it-card-link link-secondary">
                                        <span class="visually-hidden">Categoria correlata: </span><?php echo esc_html($cat->name); ?>
                                    </a>
                                <?php endif; ?>
                            </div>
    ...
    <?php else: ?>
        <div class="alert alert-warning" role="alert">Non è stato trovato alcun articolo con la categoria: <?php echo esc_html($category->name); ?></div>
    <?php endif; ?>
</section>
